...
(
    ...
    `amplitude`      decimal(8, 4)  NOT NULL DEFAULT '0.0000' COMMENT '振幅%',
    `ud_volume`      decimal(8, 4)  NOT NULL DEFAULT '0.0000' COMMENT '涨跌幅%(可为负)',
    `ud_amount`      decimal(8, 4)  NOT NULL DEFAULT '0.0000' COMMENT '涨跌额(可为负)',
    `turnover_rate`  decimal(8, 4)  NOT NULL DEFAULT '0.0000' COMMENT '换手率%',
    `create_time`    datetime       NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`    datetime       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
